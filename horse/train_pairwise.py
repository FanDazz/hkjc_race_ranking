...
parser.add_argument('--sampling_perc', default=0.5, type=float, help='Sampling percentage for pairwise learning')
...
parser.add_argument('--epoch', default=20, type=int, help='Num of epochs')
...
def split_cols(data):
    ...
    return (x, d.long(), f.long(), j.long(), h.long(), t.long())
...
def pairwise_sampler(data, sample_perc=0.5):
    ...
    data_race_participants = data.groupby(['race_key'])['dr'].max() # race: maximum participant
...
#     print(f'data_remix shape: {data_remix.shape}')
    ...
    for race_key in race_keys: # go over each race
        num_of_participants = data_race_participants[race_key]
        for ix in range(1, num_of_participants+1):  # go over each participant
...
#     print(f'Sampling data shape: {sampled_x1.shape, sampled_x2.shape}')
...
train = train.groupby(['race_key', 'pla']).first().reset_index() # remove place overlap
for ep in range(args.epoch):
    ...
    for batch_data in horse_pairwise_data_loader(Xs1, Xs2, args.batch_size, shuffle=True):
        ...
        loss = PairwiseLoss(y_pred1, y_pred2).view(-1)

        loss.sum().backward()
        opt.step()

        ep_loss.append(loss.sum().to(device('cpu')).tolist())
